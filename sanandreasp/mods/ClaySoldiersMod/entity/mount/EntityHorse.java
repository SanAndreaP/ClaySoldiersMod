...
public class EntityHorse extends EntityCreature implements IMount
{
    ...
    public EntityHorse(World world, double x, double y, double z, int i)
    {
        ...
        setHorseSpecs(i);
        renderDistanceWeight = 5D;
        worldObj.playSoundAtEntity(this, "step.gravel", 0.8F,
    ...
    public boolean attackEntityFrom(DamageSource damagesource, float i)
    {
        ...
        if (damagesource.isFireDamage()
                && this.dataWatcher.getWatchableObjectByte(18) <= 0 && hasSpec)
            specDeath = true;
        else
            specDeath = false;
        ...
        if (fred && this.getHealth() <= 0)
        {
            ...
            {
                EntityHorse entity =
                        new EntityHorse(worldObj, posX, posY, posZ,
                                this.getType());
                entity.setNightmare(true);
                entity.setAltTex(rand.nextInt(3) != 0 ? 0 : rand
                        .nextInt(Textures.HORSE_NIGHTMARE.length));
                // entity.texture =

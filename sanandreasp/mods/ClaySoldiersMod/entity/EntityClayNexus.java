...
public class EntityClayNexus extends EntityCreature
{
    ...
    public void setActive(boolean flag)
    {
        ...
        this.dataWatcher.updateObject(13, (byte) (flag ? prevDW | 1 : prevDW
                & ~1));
    }
    ...
    {
        byte prevDW = this.dataWatcher.getWatchableObjectByte(13);
        this.dataWatcher.updateObject(13, (byte) (flag ? prevDW | 2 : prevDW
                & ~2));
    }
    ...
    {
        byte prevDW = this.dataWatcher.getWatchableObjectByte(13);
        this.dataWatcher.updateObject(13, (byte) (flag ? prevDW | 4 : prevDW
                & ~4));
    }
    ...
    {
        byte prevDW = this.dataWatcher.getWatchableObjectByte(13);
        this.dataWatcher.updateObject(13, (byte) (flag ? prevDW | 8 : prevDW
                & ~8));
    }
    ...
    public void onUpdate()
    {
        ...
        if (worldObj.isBlockIndirectlyGettingPowered(cPosX, cPosY - 1, cPosZ)
                || worldObj.getStrongestIndirectPower(cPosX, cPosY, cPosZ) > 0)
            setActive(true);
        else
            setActive(false);
    }
    ...
    public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        par1EntityPlayer.openGui(CSMModRegistry.instance, 0, this.worldObj,
                this.entityId, 0, 0);
        return true;
    }
    ...
    private void spawnSoldiers()
    {
        ...
        for (int i = 0; i < getMaxSpwnSoldiers() && !isMaxSpawningReached(); i++)
        {
            ...
            {
                ec =
                        new EntityClayMan(worldObj, (int) Math.floor(clayX),
                                (int) Math.floor(clayY),
                                (int) Math.floor(clayZ), getColor());
                ec.setNexusSpawn();
            }
            ...
            if (!getSpawnMount())
            {
                ...
                if (hasRandItems())
                {
                    ...
                    do
                    {
                        ...
                        {
                            loopInd = 0;
                            if (checkIfItemIsValidAndApply(nexusIS[index], ec,
                                    false))
                            {
                                checkForUpgradeItem(ec);
                            }
        ...
        for (Entity entity : el)
        {
            if (entity instanceof EntityClayMan)
            {
                if (((EntityClayMan) entity).getClayTeam() == getColor()
                        && ((EntityClayMan) entity).spawnedFromNexus())
                    count++;
            }

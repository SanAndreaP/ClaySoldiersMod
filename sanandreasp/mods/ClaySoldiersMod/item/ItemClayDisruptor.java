...
public class ItemClayDisruptor extends Item
{
            ...
            EntityPlayer entityplayer)
    {
        if (CSMModRegistry.getWaveTime(entityplayer) <= 0)
        {
            ...
            if (!world.isRemote
                    || (FMLCommonHandler.instance().getSide().isClient() && FMLCommonHandler
                            .instance().getMinecraftServerInstance() == null))
            ...
            {
                world.playSoundAtEntity(entityplayer, "portal.trigger", 1.5F,
        ...
        for (int k = 0; k < list1.size(); k++)
        {
            ...
            if (entity1 instanceof EntityClayMan)
            {
                entity1.attackEntityFrom(
                        DamageSource.causePlayerDamage(player), 500);
            }
        ...
        if (par4 != 0)
        {
            ...
            for (int var8 = 0; var8 < var7; ++var8)
            {
                for (int var9 = 0; var9 < var7; ++var9)
                {
                    ...
                    {
                        double var11 =
                                (double) par1 + ((double) var8 + 0.5D)
                                        / (double) var7;
                        double var13 =
                                (double) par2 + ((double) var9 + 0.5D)
                                        / (double) var7;
                        double var15 =
                                (double) par3 + ((double) var10 + 0.5D)
                                        / (double) var7;
                        int var17 = rand.nextInt(6);
                        CSMModRegistry.proxy.spawnBlockParticles(0,
                                new Object[]
                                {
                                        var11, var13, var15, (double) par1,
                                        (double) par2, (double) par3, var6,
                                        par5
                                });
                    }
                }
    ...
    {
        this.itemIcon =
                par1IconRegister.registerIcon("ClaySoldiersMod:claydisruptor");
    }
}

...
public class RenderClayMan extends RenderBiped
{
    ...
    protected void renderEquipped(EntityClayMan entitycm, float f)
    {
        ...
        for (int id : entitycm.getUpgrades())
            CSMModRegistry.clayUpgRegistry.getUpgradeByID(id).onRenderEquipped(
                    entitycm, this.renderManager, f, this.mc1);
            ...
            double d2, float f, float f1)
    {
        ...
        GL11.glPushMatrix();
        for (int id : entitycm.getUpgrades())
            CSMModRegistry.clayUpgRegistry.getUpgradeByID(id).onPreRender(
                    entitycm, this.renderManager, f, this.mc1);
        
        super.doRenderLiving(entityliving, d, d1, d2, f, f1);
        
        for (int id : entitycm.getUpgrades())
            CSMModRegistry.clayUpgRegistry.getUpgradeByID(id).onPostRender(
                    entitycm, this.renderManager, f, this.mc1);
        GL11.glPopMatrix();
    }
    ...
    protected ResourceLocation getEntityTexture(Entity par1Entity)
    {
        if (par1Entity instanceof EntityClayMan)
        {
            ...
            {
                return Textures.CLAYMAN[2][clayMan.getClayTeam()][clayMan
                        .getUniqTexture()];
            }
            else if (clayMan.getRareTexture() >= 0)
            {
                return Textures.CLAYMAN[1][clayMan.getClayTeam()][clayMan
                        .getRareTexture()];
            }
            else
            {
                if (!SoldierTeams.getTeamByID(clayMan.getClayTeam()).isCustom)
                    return Textures.CLAYMAN[0][clayMan.getClayTeam()][clayMan
                            .getClayTexture()];
                else

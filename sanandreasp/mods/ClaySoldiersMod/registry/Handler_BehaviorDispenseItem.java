...
public class Handler_BehaviorDispenseItem implements IBehaviorDispenseItem
{
            ...
            ItemStack par2ItemStack)
    {
        ...
        this.func_82489_a(par1IBlockSource,
                EnumFacing.getFront(par1IBlockSource.getBlockMetadata()));
        return var3;
    }
            ...
            ItemStack par2ItemStack)
    {
        EnumFacing var3 =
                EnumFacing.getFront(par1IBlockSource.getBlockMetadata());
        IPosition var4 =
                BlockDispenser.getIPositionFromBlockSource(par1IBlockSource);
        ItemStack var5 = par2ItemStack.splitStack(1);
        func_82486_a(par1IBlockSource.getWorld(), var5, 6, var3, var4);
        return par2ItemStack;
    }
    
    public static void func_82486_a(World par0World, ItemStack par1ItemStack,
            int par2, EnumFacing par3EnumFacing, IPosition par4IPosition)
    {
        double var5 =
                par4IPosition.getX() + par3EnumFacing.getFrontOffsetX() * 0.5F;
        double var7 = par4IPosition.getY();
        double var9 =
                par4IPosition.getZ() + par3EnumFacing.getFrontOffsetZ() * 0.5F;
            ...
            double y, double z, ItemStack item)
    {
        if (item.getItem() instanceof ItemClayMan)
        {
            EntityClayMan entityclayman =
                    new EntityClayMan(world, x, y, z, item.getItemDamage());
        ...
        else if (item.getItem() instanceof ItemHorses)
        {
            EntityHorse entityhorse =
                    new EntityHorse(world, x, y, z, item.getItemDamage());
            if (item.getItem() == CSMModRegistry.pegasusDoll)
            {
                entityhorse =
                        new EntityPegasus(world, x, y, z, item.getItemDamage());
            }
            world.playSoundEffect(x, y, z, "step.gravel", 1.0F,
                    1.0F / 0.4F + 1.2F);
            return entityhorse;
        ...
        else if (item.getItem() instanceof ItemTurtle)
        {
            EntityTurtle entityturtle =
                    new EntityTurtle(world, x, y, z, item.getItemDamage());
        ...
        else if (item.getItem() instanceof ItemBunny)
        {
            EntityBunny entitybunny =
                    new EntityBunny(world, x, y, z, item.getItemDamage());
            world.playSoundEffect(x, y, z, "step.stone", 1.0F,
                    1.0F / 0.4F + 0.8F);
            return entitybunny;
        }
        else if (item.getItem() instanceof ItemGecko)
        {
            EntityGecko entitygecko =
                    new EntityGecko(world, x, y, z, item.getItemDamage());
            world.playSoundEffect(x, y, z, "step.gravel", 1.0F,
                    1.0F / 0.4F + 1.2F);
            return entitygecko;
        }
